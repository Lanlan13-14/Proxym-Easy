#!/usr/bin/env bash
# vless-x25519.sh
# 只负责 VLESS + x25519（非抗量子）入站文件（x25519_<port>.json）与对应节点记录（/etc/proxym/vless.json）
# 要求：
#  - 仅包含 x25519 相关字段（不出现 reality 或 mlkem 字段）
#  - 端口随机选择空闲端口（可由用户覆盖）
#  - UUID 与密码自动生成，节点名称自动生成：国旗 + 三字码 + 城市
#  - 自动优先检测公网 IPv4 并做地理探测
#  - 支持交互式添加（默认）与 reset（仅删除 x25519_* 文件）
set -euo pipefail
...
# x25519 inbound JSON（仅 tcp/ws 基本字段，不包含 reality/mlkem）
...
add_node_interactive(){
  ...
  # URI 仅包含 x25519 相关参数（不包含 reality/mlkem 字段）
  ...
  node_json=$(jq -n \
    --arg uuid "$uuid" \
    --arg port "$port" \
    --arg decryption "x25519.x25519.0s" \
    --arg encryption "x25519.x25519" \
    --arg ip "$pubip" \
    --arg tag "$name" \
    --arg uri "$uri" \
    --arg domain "$pubip" \
    ...
    '{
      uuid:$uuid, port:($port|tonumber), decryption:$decryption, encryption:$encryption, ip:$ip, tag:$tag, uri:$uri, domain:$domain, network:$network, path:$path, host:$host, fingerprint:$fingerprint, is_custom_tag:false, push_enabled:false, push_url:"", push_token:"", kex:$kex, method:$method, rtt:$rtt, use_mlkem:$use_mlkem }')
