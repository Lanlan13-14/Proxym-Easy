...
ensure_command() {
    ...
    case "$cmd" in
        ...
        uuidgen)
            case "$mgr" in
                apt) to_install=(uuid-runtime) ;;
                yum|dnf|zypper|pacman|apk) to_install=(util-linux) ;;
    ...
    # 执行安装
    if ! $SUDO bash -c "install_with_pkgmgr \"$mgr\" ${to_install[*]+"${to_install[*]}"}"; then
...
auto_install_deps() {
    # 需要的命令列表
    local cmds=(jq curl openssl uuidgen vim)
    for c in "${cmds[@]}"; do
...
urlencode() {
    local raw="$1"
    # 如果 jq 不可用，使用内置编码函数回退
    if command -v jq >/dev/null 2>&1; then
        jq -nr --arg v "$raw" '$v|@uri'
    else
        # 简单实现：对非安全字符进行百分号编码
        local i ch hex encoded=""
        for ((i=0; i<${#raw}; i++)); do
            ch="${raw:$i:1}"
            case "$ch" in
...
gen_pass() {
    # 优先使用 openssl 生成高熵数据并过滤字符集，最终取 50 字符
    local pass
    if command -v openssl >/dev/null 2>&1; then
        pass=$(openssl rand -base64 64 | tr -dc 'A-Za-z0-9!@#$%^&*()_+=\-\[\]{}|;:,.<>?/' | head -c 50 || true)
    fi
    if [[ -z "${pass:-}" ]]; then
        pass=$(tr -dc 'A-Za-z0-9!@#$%^&*()_+=\-\[\]{}|;:,.<>?/' </dev/urandom | head -c 50 || true)
    fi
    # 如果仍然为空，退回到可打印字符
    if [[ -z "${pass:-}" ]]; then
...
reset_credentials() {
    ...
    if command -v jq >/dev/null 2>&1; then
        ...
        # 没有 jq 的情况下，使用简单的替换（风险较高），先备份
        cp "$file" "${file}.bak"
        # 尝试插入 accounts 块或替换已有 user/pass（尽量稳健）
        if grep -q '"accounts"' "$file"; then
            # 替换 user 和 pass 字段
            sed -E "s/\"user\"[[:space:]]*:[[:space:]]*\"[^\"]*\"/\"user\": \"${new_user//\"/\\\"}\"/g; s/\"pass\"[[:space:]]*:[[:space:]]*\"[^\"]*\"/\"pass\": \"${new_pass//\"/\\\"}\"/g" "${file}.bak" > "$file"
        else
            # 在 settings 中插入 accounts 数组（在 "udp": ... 之前插入）
            awk -v u="$new_user" -v p="$new_pass" '
            BEGIN{insert=0}
            /"settings":/ {print; getline; print; insert=1; next}
            {
              if(insert && /"udp"/){
                print "        \"accounts\": ["
                print "          {"
