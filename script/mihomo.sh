...
arch=""                   # 系统架构
arch_raw=""               # 原始架构
GITHUB_PROXY=""           # GitHub 加速前缀
GITHUB_PROXY_FILE="/root/.mihomo_github_proxy"  # 持久化文件
...
load_github_proxy() {
    if [[ -f "$GITHUB_PROXY_FILE" ]]; then
        ...
        if [[ -n "$GITHUB_PROXY" ]]; then
            echo -e "${green}已加载 GitHub 加速镜像: ${GITHUB_PROXY}${reset}"
        fi
    fi
}
...
# 保存 GitHub 加速前缀
save_github_proxy() {
    echo "$GITHUB_PROXY" > "$GITHUB_PROXY_FILE"
}
...
config_github_proxy() {
    echo "================================="
    echo -e "${green}GitHub 加速镜像配置${reset}"
    echo "---------------------------------"
    echo -e "${yellow}当前镜像地址: ${GITHUB_PROXY:-未设置}${reset}"
    echo "---------------------------------"
    echo -e "${green}1${reset}. 设置镜像地址"
    echo -e "${green}2${reset}. 清除镜像地址"
    echo -e "${green}3${reset}. 使用内置 CDN 代理 (gh-proxy.com)"
    echo -e "${green}0${reset}. 返回主菜单"
    echo "================================="
    read -rp "$(echo -e "${yellow}请输入选项: ${reset}")" proxy_choice
    
    case "$proxy_choice" in
        1)
            read -rp "$(echo -e "${green}请输入 GitHub 加速镜像地址 (例如: https://gh-proxy.com/): ${reset}")" new_proxy
            if [[ -n "$new_proxy" ]]; then
                # 确保地址以 / 结尾
                [[ "$new_proxy" != */ ]] && new_proxy="${new_proxy}/"
                GITHUB_PROXY="$new_proxy"
                save_github_proxy
                echo -e "${green}镜像地址已设置: ${GITHUB_PROXY}${reset}"
            else
        ...
        2)
            GITHUB_PROXY=""
            save_github_proxy
            echo -e "${green}已清除镜像地址${reset}"
            ;;
        3)
            GITHUB_PROXY="https://gh-proxy.com/"
            save_github_proxy
            echo -e "${green}已设置内置 CDN 代理: ${GITHUB_PROXY}${reset}"
            ;;
        0)
            menu
            return
...
get_url() {
    local url="$1"
    local candidate
    local proxied_url=$(add_proxy "$url")
...
show_status() {
    ...
    echo -e "GitHub镜像: ${green}${GITHUB_PROXY:-未设置}${reset}"
}
...
menu() {
    ...
    echo -e "${green}99${reset}. 更新脚本"
    echo -e "${green}98${reset}. 配置GitHub镜像"
    ...
    case "$input" in
        ...
        20) config_mihomo ;;
        30) logs_mihomo ;;
        98) config_github_proxy ;;
        99) update_shell ;;
        0) 
            echo -e "${green}下次使用请输入: mihomo${reset}"
            exit 0 
            ;;
        *) 
