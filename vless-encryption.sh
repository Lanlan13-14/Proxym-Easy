...
function generate_config() {
    ...
    if [ "$use_reality" = false ]; then
        ...
        # REALITY 配置
        log "生成 X25519 密钥对 (用于 REALITY)..."
        x25519_output=$(xray x25519)
        private=$(echo "$x25519_output" | grep "PrivateKey:" | cut -d ':' -f2- | sed 's/^ *//;s/ *$//' | xargs)
        public_key_base64=$(echo "$x25519_output" | grep "Password:" | cut -d ':' -f2- | sed 's/^ *//;s/ *$//' | xargs)
        if [ -z "$private" ] || [ -z "$public_key_base64" ]; then
            error "密钥生成失败。"
        fi
        decryption="none"
        ...
        shortId="${shortids[0]}"
        log "REALITY 配置: dest=$dest, sni=$sni, shortId=$shortId"
    fi
    ...
    if [ "$use_reality" = true ]; then
        ...
        uri_params="type=${type_uri}&flow=${flow}&security=${security_uri}&sni=${sni}&fp=chrome&sid=${shortId}&pbk=${public_key_base64}"
        domain=""
    else
        echo "请选择传输层:"
        echo "[1] TCP (默认)"
