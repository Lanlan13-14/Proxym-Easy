...
6. 为 `ALLOWED_IPS` 添加 main-table 回程策略，避免 WARP 抢走客户端回包。
7. 之后才启动 SmartDNS 和 sniproxy。

WARP 失联时 sniproxy 会立即停止、容器退出并由 Docker 重启，**不会回退为 VPS 直连出口**。
